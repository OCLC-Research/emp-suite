 ...
 *
 */
public class CiteXtractor extends NerBase {
	 ...
	 */
	private CiteXtractor() { }

	public static CiteXtractor getInstance() throws IllegalArgumentException {
		return getInstance(defaultConfiguration);
	}

	public static CiteXtractor getInstance(Map<String, String> config) throws IllegalArgumentException {
		CiteXtractor tagger = new CiteXtractor();

		if ( null == config ) {
			throw new IllegalArgumentException("NULL configuration in CiteExtractor.");
		}

		if ( ! tagger.isConfigurationComplete(config) ) {
			throw new IllegalArgumentException("Incomplete configuration in CiteXtractor.");
		}
